"""Writing a blank Jastrow factor into a `correlation.data`.
...
that is what the optimisation is for, and a Jastrow with no coefficients is exactly how one
starts. CASINO reads such a file happily: `read_u_term` in `pjastrow.f90` fills `alpha` with
...
writes it, and everything it writes is a value CASINO's reader was seen to accept:
    ...
    u       one set, spherical harmonic l=m=0, `N_u` terms, one cutoff
    chi     one set per element, every atom labelled, `N_chi` terms, one cutoff per set
    f       one set per element, `N_f_eN` * `N_f_ee` terms, one cutoff per set

Cutoffs are written as zero on purpose. `read_u_term` reads a cutoff of zero as "use the
default" and calls `default_L_u()`, which is 2 a.u. for a single atom, 5 a.u. for a molecule,
and the Wigner-Seitz radius of the cell for a periodic system -- a rule that depends on the
geometry, which means writing a number here instead would be reimplementing CASINO's own
choice and getting it wrong for the next system. `describe` says out loud what the zero means.
...
# take back. The defaults are the ones CASINO's own examples use, with two exceptions, both
# noted where they are written: the cutoffs, which are left to CASINO, and the chi cusp, which
# is off because every basis this can write for either satisfies the cusp condition already or
# forbids imposing it here.
DEFAULTS = {
...
def blank(geometry: dict, terms=TERMS, title: str = '', settings: dict | None = None) -> str:
    """A whole `correlation.data` holding one blank Jastrow factor.
    ...
    return '\n'.join(header) + '\n' + jastrow(geometry, terms=terms, title=title, settings=settings)


def check(geometry: dict, terms=TERMS, settings: dict | None = None, pseudo: set[int] | None = None, basis: str = '') -> list[str]:
    """Everything CASINO would errstop on, found before the file is written.

    Each of these is a line in `read_u_term`, `read_chi_term` or `read_f_term` that ends the run
    -- and the run ends after the queue has already started it, which is the whole reason for
    checking here.
    """
    settings = settings if settings is not None else settings_for()
    ...
    if not terms:
        errors.append('no terms asked for: a Jastrow factor with no terms is not a Jastrow factor')
    ...
    if not geometry['atomic_numbers'] and ({'chi', 'f'} & set(terms)):
        errors.append('the chi and f terms are electron-nucleus terms, and this system has no atoms')
    if settings['trunc_order'] < 2 and settings['optimizable']:
...
def defaulted_cutoff(term: str, geometry: dict, settings: dict) -> float | None:
    ...
    `default_L_u`, `default_L_chi` and `default_L_f` in `pjastrow.f90`, and only the branch this
    module can write for: a periodic cell's default is a fraction of its Wigner-Seitz radius,
    which needs the lattice vectors, and a truncation order below 2 is refused before this.
    This is reported, never written -- the file says zero, and CASINO decides.
    """
    if geometry['periodicity'] or settings['trunc_order'] < 2:
        return None
    if term == 'u':
        return 2.0 if len(geometry['atomic_numbers']) == 1 else 5.0
    return {'chi': 4.0, 'f': 3.0}.get(term)


def describe(geometry: dict, terms=TERMS, settings: dict | None = None, pseudo: set[int] | None = None, basis: str = '') -> list[str]:
    """What was written that the file does not say in so many words."""
    settings = settings if settings is not None else settings_for()
    pseudo = pseudo or set()
    notes = []
    zeroed = [name for name in ('cutoff_u', 'cutoff_chi', 'cutoff_f') if name.split('_')[1] in terms and not settings[name]]
    if zeroed:
